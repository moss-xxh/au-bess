        ...
        function updateStatsTableDisplay() {
            ...
            pageData.forEach(stat => {
                ...
                const dispatchedLabel = isDispatched ? 'Y' : 'N';
                const dispatchedColor = isDispatched ? '#34c759' : '#ff3b30';
                const dispatchedBg = isDispatched ? 'rgba(52,199,89,0.15)' : 'rgba(255,59,48,0.15)';
                const dispatchedBorder = isDispatched ? 'rgba(52,199,89,0.35)' : 'rgba(255,59,48,0.35)';

                row.innerHTML = `
                    <td style="white-space:nowrap; font-weight: 500;">${stat.date}</td>
                    <td>${stat.station}</td>
                    <td style="text-align: center;"><span class="status-badge" style="background:${dispatchedBg}; color:${dispatchedColor}; border:1px solid ${dispatchedBorder}; font-weight:700; letter-spacing:1px;">${dispatchedLabel}</span></td>
                    <td>
                        <a href="#" class="action-link" style="color: #00ff88;" onclick="viewStationDailyDetails('${stat.date}', '${stat.station}', '${stat.operationMode}'); return false;"><span data-i18n="operationLog.dailyStats.detail">详情</span></a>
                    </td>
                `;
        ...
        function updateSortIndicators() {
            ...
            const columns = ['date', 'station', 'dispatched', 'operationMode'];
            columns.forEach(col => {
        ...
        function applyOpFilters() {
            ...
            filteredData = allDispatchRecords.filter(record => {
                ...
                if (startDate && record.time.slice(0, 10) < startDate) return false;
                if (endDate && record.time.slice(0, 10) > endDate) return false;
                return true;
            });
            ...
            // Apply dispatched filter on stats level
            if (dispatched === 'Y') {
                dailyStatsData = dailyStatsData.filter(s => s.chargeCount > 0 || s.dischargeCount > 0);
        ...
        function resetOpFilters() {
            ...
            const s = document.getElementById('opStartDate'); if (s) s.value = '';
            const e = document.getElementById('opEndDate');   if (e) e.value = '';
        ...
        // Filtering is triggered by 查询 button only
        function setupOpRealTimeSearch() {
            // no-op: all filtering triggered by 查询 button click
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (window.i18n) {
                window.i18n.addObserver((newLang) => {
                    ...
                    if (window.i18n.updatePageTexts) window.i18n.updatePageTexts();
                    updateStatsTableDisplay();
                    updateStatsPaginationDisplay();
                });
            }
